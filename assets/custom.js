...
// Size chart modals
const initializeSizeCharts = () => {
  const sizeCharts = document.querySelectorAll('[data-chart-modal]');

  sizeCharts.forEach((sizeChart) => {
    const id = sizeChart.dataset.chartModal;
    const open = document.querySelectorAll(`[data-chart-open="${id}"]`);
    const close = document.querySelectorAll(`[data-chart-close="${id}"]`);

    open.forEach((el) => {
      el.addEventListener('click', (e) => {
        e.preventDefault();
        sizeChart.classList.add('is-active');
        htmlEl.classList.add('lock-all');
      });
    });
    ...
    close.forEach((el) => {
      el.addEventListener('click', (e) => {
        ...
        htmlEl.classList.remove('lock-all');
      });
    });
  });
};
