...
`;
  ...
  var FlickityCarousel = class extends CustomHTMLElement {
    ...
    // this removes the 'left' and 'right' arrows on the featured image on hover
  ...
  var FlickityControls = class extends CustomHTMLElement {
    ...
    get flickityCarousel() {
      return (this._flickityCarousel =
        ...
        document.getElementById(this.getAttribute('controls'))); // 'controls' the id that this control attribute has/ is equal to
    }
  ...
  var ProductRecommendations = class extends HTMLElement {
    ...
    async loadProducts(intent = this.intent) {
      const response = await fetch(
        `${window.themeVariables.routes.productRecommendationsUrl}?product_id=${this.productId}&limit=${this.recommendationsCount}&section_id=${this.sectionId}&intent=${intent}`
      );
      const div = document.createElement('div');
      div.innerHTML = await response.text();
      ...
      if (productRecommendationsElement.hasChildNodes()) {
        ...
        setTimeout(() => {
          setTimeout(() => {
            ...
            // we need to insert the quickbuy overlays before the flickity carousel or else
            // the overlay will be constraint by the height of the carousel (size of product recommendations)
            const flickityCarouesel = this.querySelector('flickity-carousel');
            const quickBuyDrawers = Array.from(
              this.querySelectorAll('quick-buy-drawer')
            );
            const quickBuyPopovers = Array.from(
              this.querySelectorAll('quick-buy-popover')
            );

            quickBuyDrawers.forEach((quickBuyDrawer) => {
              flickityCarouesel?.before(quickBuyDrawer);
            });

            quickBuyPopovers.forEach((quickBuyPopover) => {
              flickityCarouesel?.before(quickBuyPopover);
            });
          }, 25);
        }, 225);
      } else {
        if (this.intent === 'complementary') {
          this.loadProducts('related');
        }
      }
  ...
  var SlideshowItem = class extends HTMLElement {
    ...
    _reloadVideo(transitionType, nextItem, pageDots) {
      const video = this.querySelector('video');
      video.play();

      // when the video ends
      video.addEventListener('ended', () => {
        // transition to 'leave' the current slide
        this.transitionToLeave(transitionType);
        // transition to 'enter' the new one
        nextItem.transitionToEnter(transitionType);
        // move the 'dot' to the next item
        pageDots.selectedIndex = nextItem.index;
      });
    }

    _stopVideo(previousSlide) {
      const video = this.querySelector('video');
      const muteButton = previousSlide.querySelector('.hero__video-mute');
      // if the button has 'is-active' it means the video is playing with volume
      if (muteButton?.classList.contains('is-active')) {
        // remove the class (for ui purposes because of how the mute/unmute button is being being rendered in slideshow.liquid)
        // i.e show unmute buttom/volume if 'is-active' is present
        muteButton.classList.remove('is-active');
        // mute the video
        video.muted = true;
      }

      // reset back to 0 so that the video can be played from the beginning when the slide is back
      video.pause();
      video.currentTime = 0;
    }
  };
  ...
  var Slideshow = class extends CustomHTMLElement {
    ...
    async select(index, shouldTransition = true, reverseDirection = false) {
      ...
      // if previous was a video, stop it
      if (previousItem.querySelector('video')) {
        previousItem._stopVideo(previousItem);
      }
      // load the video on the new item if it contains one
      if (newItem.querySelector('video')) {
        // if it's a video, we want to go to the next item whenever it ends
        const automaticNextItem = this.items[(index + 1) % this.items.length];
        newItem._reloadVideo(
          this.transitionType,
          automaticNextItem,
          this.pageDots
        );
  ...
  var GiftCardRecipient = class extends HTMLElement {
    connectedCallback() {
      ...
      (_a = this.recipientCheckbox) == null
        ? void 0
        : _a.addEventListener('change', this._synchronizeProperties.bind(this));
  ...
  var ProductImageZoom = class extends OpenableElement {
    ...
    async _buildItems() {
      ...
      // the current media for this product
      const activeProductMediaIds = product['media'].map((media) => media.id);

      // all the medias from the carousel
      let activeImages = Array.from(
        this.mediaElement.querySelectorAll(
          '.product__media-item[data-media-type="image"]:not(.is-filtered)'
        )
      ).filter((item) => {
        // activeImages should only contain images for this product handle / matches the media id's
        const activeImageId = parseInt(item.getAttribute('data-media-id'));
        if (activeProductMediaIds.includes(activeImageId)) {
          return item;
        }
      });
  ...
  var ProductInventory = class extends HTMLElement {
    ...
    _onVariantChanged(event) {
      ...
      // only show stock amount when and after a size is selected
      if (
  ...
  var PaymentContainer = class extends HTMLElement {
    ...
    _onVariantChanged(event) {
      ...
      this._updateNotifyMeButton(event.detail.variant);
    }
    _updateAddToCartButton(variant) {
      ...
      if (!variant) {
        ...
        if (variant['available']) {
          ...
          addToCartButtonElement.classList.add('button--ternary');
          addToCartButtonText = comingSoon
    ...
    _updateNotifyMeButton(variant) {
      ...
      if (form) {
        if (variant?.id && form.variant) {
          form.variant.value = variant.id;
        }

        const productID = variant?.product_id;
        if (productID && form.product) {
  ...
  var ProductForm = class extends HTMLFormElement {
    ...
    async _onSubmit(event) {
      ...
      if (productForm.get('id')) {
        ...
        const distance =
          document.querySelector('.product-form__variants')?.offsetTop || false;
  ...
  var ProductMedia = class extends CustomHTMLElement {
    ...
    async _onVariantChanged(event) {
      ...
      if (
        ...
        // this is what is actually changing the 'main' image
  ...
  window.customElements.define('notify-me', NotifyMe);
    ...
    _updatePrices(variant) {
      ...
      if (!variant) {
        ...
        if (variant['compare_at_price'] > variant['price']) {
          ...
          productPrices.innerHTML += `
            <span class="price price--highlight ${this.priceClass}">
              <span class="visually-hidden">
          ...
          productPrices.innerHTML += `
            <span class="price ${this.priceClass}">
              <span class="visually-hidden">
                ${window.themeVariables.strings.productSalePrice}
              </span>
              ${formatMoney(variant['price'], currencyFormat)}
  ...
  var ProductVariants = class extends CustomHTMLElement {
    async connectedCallback() {
      /* this finds inside of the <product-variant> an attribute with the value of 'form-id' i.e: 'product-form-template--21330202853657__main-6841318211638'
       we then use this id to find an element with that id; then we find the attribute value of 'id' -> current line 634 in 'product-form/liquid'
      ...
      */
      ...
      // gets the input element which contains a "value" attribute/property where the actual value is the variant id
      this.masterSelector = document.getElementById(
        this.getAttribute('form-id')
      ).id;
      this.masterFinalSale = document.getElementById(
        this.getAttribute('form-id')
      )['properties[Sale]'];
      ...
      this.product['variants'] = JSON.parse(
        document.querySelector(`#VariantsJson-${this.product.id}`).innerHTML
      );
      this.delegate.on(
        'change',
        '[name^="option"]',
    ...
    displayProductGallery({ product_id } = {}) {
      if (product_id) {
        document
    ...
    selectVariant(id) {
      ...
      if (this.updateUrl && history.replaceState) {
        ...
        const selectedVariantHandle = selectedVariant.url
          .split('?')[0]
          .split('/')[2];
        const currentVariantHandle = window.location.href
          .split('?')[0]
          .split('/')[4];
      ...
      this._updateDisableSelectors(variant);
      ...
      if (options.length === variant.options.length) {
        ...
        triggerEvent(this.masterSelector.form, 'variant:selected', {
    ...
    _updateFinalSale(variant) {
      this.querySelectorAll('.final-sale').forEach((el) => {
        ...
        const isFinalSale = Boolean(
          colors.includes('all') || colors.includes(variant.option1)
        );
        el.style.display = isFinalSale ? 'inline' : 'none';
        this.masterFinalSale.value = isFinalSale ? 'Final' : '';
      });
    }

    // this method updates the 'selected' elements/swatches in the product form
    ...
    _getVariantFromOptions() {
      ...
      return this.product.variants.find((variant) => {
        return options.reduce((matched, option, index) => {
          const optionName = `option${index + 1}`;
          return matched && variant[optionName] === option;
        }, true);
      });
    }
    ...
    _getSelectedOptionValues() {
      ...
      // if we only choose color and don't have size yet so it doesnt trigger a change event...
      // this.mainProductId == this.currentProductId makes sure that we only change url if a variant change is done on the pdp
      if (options.length === 1 && this.mainProductId == this.currentProductId) {
        const color = options[0];
        const variants = this.product.variants;
        // option1 is used to store the color
    ...
    _updateProductRecommendations(selectedVariant) {
      const { product_id: currentProductId } = selectedVariant.featured_image;
      if (currentProductId) {
        document.querySelectorAll('product-recommendations').forEach((el) => {
          el.productId = currentProductId;
        });
      }
    }

    _updateDisableSelectors(variant) {
      const selectedVariant = variant || this.selectedVariant;
      if (!selectedVariant) {
        return;
      }
      ...
      if (this.optionSelectors && this.optionSelectors[0]) {
        this.product['options'][0]['values'].forEach((value, valueIndex) => {
          ...
          // this.optionSelectors[1] = size selector
          if (this.optionSelectors[1]) {
            // an array of all of the sizes ex: [XS, S, M, ...]
            const allSizeOptions = this.product['options'][1]['values'];

            allSizeOptions.forEach((optionSize, valueIndex2) => {
              const allPossibleVariants = this.product['variants'];
              // option1 = color option & option2 = size option
              const hasAtLeastOneCombination2 = allPossibleVariants.some(
                (variant) =>
                  variant['option2'] === optionSize &&
                  variant['option1'] === selectedVariant['option1'] &&
                  variant
              );
              const hasAvailableVariant2 = allPossibleVariants.some(
                (variant) =>
                  variant['option2'] === optionSize &&
                  variant['option1'] === selectedVariant['option1'] &&
                  variant['available']
              ...
              if (this.optionSelectors[2]) {
                this.product['options'][2]['values'].forEach(
                  (value3, valueIndex3) => {
                    const hasAtLeastOneCombination3 = allPossibleVariants.some(
                        (variant) =>
  ...
  var CartNotification = class extends CustomHTMLElement {
    ...
    startTimer() {
      //3e3
      this._timeout = setTimeout(() => (this.hidden = true), 5000);
    }
    _onShow(event) {
  ...
  var ProductStickyForm = class extends HTMLElement {
    connectedCallback() {
      ...
      this.compareAtPriceElement = this.querySelector(
        '.product-sticky-form__compare-at-price'
      );
    ...
    _onVariantChanged(event) {
      ...
      if (
        this.compareAtPriceElement &&
        variant['compare_at_price'] > variant['price']
      ) {
        this.compareAtPriceElement.style.display = 'inline';
        this.compareAtPriceElement.innerHTML = formatMoney(
          variant['compare_at_price'],
          currencyFormat
        );
      } else if (this.compareAtPriceElement) {
  ...
  (() => {
    ...
    function redeemDisplay() {
      const redeemEls = document.querySelectorAll('[data-redeem]');
      const balance = customerDetails.pointsBalance || 0;

      redeemEls.forEach((el) => {
        const price = parseInt(el.dataset.redeem);
        el.style.display = balance >= price ? 'inline' : 'none';
      });
    }
    ...
    function tierDisplay() {
      try {
        ...
        const tier = customerDetails.vipTier.id;

        tierEls.forEach((el) => {
          const elTier = el.dataset.tier;
          el.classList.toggle('is-active-tier', elTier == tier);
        });
      } catch (e) {
        console.warn('tierDisplay: No tier available for customer.', e);
      }
    }
